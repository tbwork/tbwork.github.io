...
本文主要通过半翻译半总结的方式介绍了Mysql的索引基础知识，先从总体上讨论了`Mysql是如何使用索引的`，然后对`组合索引`进行了一些探讨，最后给出了一些实际数据库设计中索引设计相关的建议。这些知识并不复杂，不需要专业的数据库学习经验就能搞明白，理解了这些可以帮助开发人员更好的进行数据库索引设计和SQL查询语句的编写。
...
本文不打算去赘述B树的原理和创建过程，有兴趣的可以[点击B树了解](https://baike.baidu.com/item/B%E6%A0%91/5411672)。假设现在索引已经创建完毕了，那么Mysql是如何查找到我们需要的数据的呢？为了更好的说明这个过程，我们先来看下下方这张`B树`结构图（网上有很多这样的图，但令人满意的很少，要么索引值是数字，主键也是数字，令新手犯晕；要么就是没有画与数据行是怎么关联的；要么就是没有重复键值，新手仍然一头雾水）。
...
```
那么Mysql索引查找器将会从根节点开始寻找“C”，C在“A-J”之间，搜索指针将会跳转到N2节点，继续查找发现在“A-E”之间，对应了N4节点，N4节点已经是根节点，在N4的第2个位置找到了C节点，共发现2条记录：9和20。下面就是如何读取9和20这两条数据了，由于计算机内存有限，且较硬盘来说小很多，也宝贵的多。所以一般情况下Mysql只把索引加载进内存，而数据块放在了硬盘中，图中为了表达清晰，叶子节点中存放的仅是主键的ID，实际情况时存放的还有记录所在的硬盘地址。知道了数据在磁盘中的存放地址后，就可以快速定位和获取数据了。
...
* 4.查询语句匹配到的索引对应的基数太小（对应`SHOW INDEX FROM table_name`结果中的`Cardinality` )，并且此时又有其他列上的查询条件。所谓基数就是表中某列所有值的取值种数，比如一张表有5行，某一列对应的值分别为：1,2,3,3,2。那么该列`基数`就是3，因为一共有三种取值：1,2,3。 如果某个单列索引的基数很小，那么我们就要考虑这个索引创建的是否合适。因为基数过小也可能导致该索引失效。基数小，意味着该索引中每一个满足条件的索引值对应的记录数很大，在这个索引值对应的记录数中再去一个个的检查其他列上的条件是否满足，整个过程总体的查找速度还未必有全表扫描来得快。
...
[6] [4] [5] -->2
```
当然了，这里是从上至下的排序，真正Mysql使用的索引数据结构一般是是树状的，但本质也是排序好的。
...
```
这时候上述三种查询都会使用到该索引。
2. 创建联合索引时，值差异化大的列放在前面，而不是那些取值种类很少的列。比如`用户名`和`用户状态`两列，创建索引的顺序应当是index(用户名、用户状态)，而不是index(用户状态、用户名)。这里涉及到一个“潜规则”，如果把基数很小的用户状态放在第一个，那么如果恰好查询语句条件恰好是（用户名，用户状态）时，用户名恰好也也命中了索引。然而当查询语句里是（用户状态，其他字段）时，Mysql优化器很可能会将其退化为全表扫描。
