...
（2）好了，到此为止，用户的请求来到了IDC-1的Spanner集群服务器上，Spanner从内存中读取到了路由配置，知道了这个请求的主体用户C所属的RZ2*不在本IDC，于是直接转到了IDC-2进行处理。

（3）进入IDC-2之后，根据流量配比规则，该请求被分配到了RZ3B进行处理。

（4）RZ2B(或者RZ2A)得到请求后对数据分区c进行访问。
...
很多同学肯定听说过这样的描述——PAXOS是唯一能解决分布式一致性问题的解法。实际上PAXOS并没有逃离CAP魔咒，它实现强一致的后果是处于弱势分区的节点将无法再提供写入服务（或者一致性读服务）。
