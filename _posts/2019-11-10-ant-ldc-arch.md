...
* 可用性分区容忍性：假设单节点是分布式的一种特例，并且分区情况下恰好是该节点挂了，这时候没有其他节点提供服务，系统也就不可用了，所以可用性分区容忍性不满足。
* 一致性分区容忍性：假设单节点是分布式的一种特例，并且分区情况下恰好是该节点挂了，这时候没有其他节点提供服务，也不存在一致性的问题，单点单机的最大好处就是一致性可以得到保障。
...
* 可用性分区容忍性：OB事务只需要同步到（N/2)+1个节点，允许其余的一小半节点分区（宕机、断网等），只要(N/2)+1个节点活着就是可用的。极端情况下，当5个节点分成2:2:1共3个分区时整个系统就确实不可用了，只是这种情况概率比较低。 
